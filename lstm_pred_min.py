...
import numpy as np
import time
from lstm_use2 import lstm_predict
...
def main():
    ...
    while rodando:
        ...
        predictions = lstm_predict(carro.history, 20, model_path="lstm_model_deltas.pt")
        # para desenhar, converte para int
        for i in range(1, len(predictions)):
            p0 = (int(predictions[i-1][0]), int(predictions[i-1][1]))
            p1 = (int(predictions[i][0]), int(predictions[i][1]))
            pygame.draw.line(tela, VERDE, p0, p1, 13)
